...
export function StudentList({ crtData, handleDeletebtn }) {
  ...
  return (
    <tr className={`border-b border-gray-200 ${crtData.isPresent ? "bg-green-100" : "bg-red-100"}`}>
      ...
      <td className={`py-3 px-6 text-center text-xl flex item-center justify-center ${crtData.isPresent ? "text-green-500" : "text-red-500"}`}>{crtData.isPresent ? <MdCheck /> : <MdClose />}</td>
